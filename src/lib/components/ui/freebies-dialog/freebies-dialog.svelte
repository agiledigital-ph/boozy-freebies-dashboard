<script lang="ts">
	...
	let _selectedFreebies: string[] = [];

	onMount(() => {
		unsubscribeFreebieFormStore = freebieFormStore.subscribe((currentState: FreebieFormState) => {
			const { isFreebieDialogOpen, selectedFreebies } = currentState;
			if (selectedFreebies) {
				_selectedFreebies = selectedFreebies.map((val) => val.name);
			}
			isDialogOpen = isFreebieDialogOpen;
		});
	});
	...
	const handleOpenChange = (open: boolean) => {
		if (!open) {
			freebieFormStore.closeDialog('freebies');
		}
	};

	const searchProducts = async (searchQuery: string) => {
		const response = await fetch(`/api/shopify/freebies?searchQuery=${searchQuery}&limit=10`);

		const responseData = await response.json();

		if (responseData.freebies.length > 0) {
			freebieFormStore.setFreebies(responseData.freebies);
		}
	};
</script>
...
<Command.Dialog open={isDialogOpen} onOpenChange={handleOpenChange} shouldFilter={false}>
	...
	<Command.List>
		...
		<Command.Group heading="Suggestions">
			{#each $freebieFormStore.freebies as product}
				<Command.Item
					value={product.name}
					onSelect={(currentValue) => {
						if (_selectedFreebies.includes(currentValue)) {
							return;
						}
						freebieFormStore.selectFreebie(currentValue);
					}}
				>
					<Check
						class={cn(
							'mr-2 h-4 w-4',
							!_selectedFreebies.includes(product.name) && 'text-transparent'
						)}
					/>
					{product.name}
				</Command.Item>
